...
To evaluate the finte sample performance of the two proposed method, 
 . The data generation process (DGP) is taken 
from the DGP described in Section 5.1 of \citeA{blackwellEffectPoliticalAdvertising2024}. In this 
...
the treatments solely for the sake of simplicity. By doing this, the ground truth 
values for the causal targets of interests conincide wit the DGP parameter. 
...
when $p=2$ and when $p=4$, $\gamma = (1, 0.5, 1, 1)^{\top}$. You can the the graphical model of this dgp in figure 
\ref{fig:dgp-dag}.
\begin{figure}[ht]
    \centering
...
\section{Estimation}
\label{estimation}
For defining our causal targets, first we need to specify the causal questions of interest. The two question of interest are: 
What is the average effect of the treatment received at the final time on the 
final outcome for any fixed set of the last $K$ treatments before $T$: 
$\mathbb{E}\left[ Y_{iT}\left(a_T = 1, S_k(\bar{A}_{T-K:T-1})\right) - Y_{iT}\left(a_T = 0, S_k(\bar{A}_{T-K:T-1})\right) \right]$, and the 
average effect of receiving one additional treatment in the last three time points, 
before $T$ on the final outcome, conditioned on  
the last treatment. To express 
this mathematically, we first define $S_k(\bar{a}_{T-K:T-1}) := \sum_{t=T-K}^{T-1} a_t \in \{0,1,2,\dots, K\}.$ as the sum 
...
$\mathbb{E}\left[Y_{iT}\left({S_k(\bar{a}_{T-K:T-1}) = s, A_{iT}}\right) - Y_{iT}\left({S_k(\bar{a}_{T-K:T-1}) = s+1, A_{iT}}\right)\right], s \in \{0,1,\dots,K-1\}$. 
To answer both of these causal questions, a marginal structural model is prposed  
that relates the assigned treatments to the expected potential outcome at the final time: 
$\mathbb{E\left[Y_{iT}\left(\bar{a}_{T-k:T}\right)\right]} = g(\bar{a}_{T-k:T}, \gamma_0)$, where 
$g(\bar{a}_{T-k:T}, \gamma_0)$ is the true unkown function parameterized by $\gamma_0$. Our job is 
to first propose a function $f(\bar{a}_{T-k:T}, \gamma)$ and then estimate its parameters $\gamma$. In this 
...
Y_{iT}\!\left(\bar a_{T-k:T}\right)
&= U_i + \tau_F a_T + \tau_C \sum_{t=T-3}^{T-1} a_t + \gamma^\top \bar X_{iT} + \epsilon_{iT} \\
\implies\quad
\mathbb{E}\!\left[Y_{iT}\!\left(\bar a_{T-k:T}\right)\right]
...
\implies\quad
\mathbb{E}\!\left[Y_{iT}\!\left(\bar a_{T-k:T}\right)\right]
&= \alpha_0 + \tau_F a_T + \tau_C s_3. %\qquad s_3 := \sum_{t=T-3}^{T-1} a_t 
\end{align}

Similary, it can be shown: 
\begin{align}
\label{eq:tau_f}
&\mathbb{E}\!\left[Y_{iT}\!\left(a_{T},S_3\right)\right]
= \alpha_0 + \tau_F a_T + \tau_C S_3 \\
&\mathbb{E}\!\left[Y_{iT}\!\left(s_3,\bar A_T\right)\right]
= \alpha_0 + \tau_F A_T + \tau_C s_3
\label{eq:tau_c}
\end{align}

\hl{The important point to note here is that since $X_{it}$ is 
exogenous to the treatment, there are no
terms involving $X_{it}(\underline{a}_{T-k})$ which are 
counterfactual covariates under 
the specified treatment regime.} 
From Eq.~\eqref{eq:tau_f} and Eq.~\eqref{eq:tau_c}, it can be seen: 

\begin{equation}
        \mathbb{E}\left[ Y_{iT}\left(a_T = 1, S_3\right) - Y_{iT}\left(a_T = 0, S_3\right) \right] = \tau_F
\end{equation}
...
\begin{equation}
    \mathbb{E}\left[Y_{iT}\left({S_3 = s, A_{iT}}\right) - Y_{iT}\left({S_3 = s+1, A_{iT}}\right)\right] = \tau_C
\end{equation}
The equality between these causal effects and the parameters of the DGP is a result of the linearity of the DGP and the absence of treatment-confounder feedback. 
...
we specify $f\left(\underline{a}_{T-k}; \gamma \right)$ as the correctly specified linear MSM:
\begin{equation}
