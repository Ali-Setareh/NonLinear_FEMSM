...
For the FE-MSM method, the parameters are estimated by solving the following optimization problem:
\begin{equation}
...
For the sequential GPLVM method, a $\text{CMM}(1)$ with a one dimensional latent variable 
(i.e., $Z_i \in \mathbb{R}$ amd $|\mathcal{Z}| = 1$) 
...
\]
where $\nu$ is the variance hyperparameter. The parameters of the MSM are estimated by
