...
\paragraph{Treaming weights.} The unit-specific fixed effects and the individual 
...
more bias than the naive IPTW for the strong unmeasured confounding case (Figure~\ref{fig:simulation_results_a2}, panels (A)--(B)) 
...
the larger estimated standard errors in this setting do not generally keep the coverage close to the nominal level. In particular, for the shortest treatment sequence ($\rho = 50$) under the stronger unmeasured confounding setting (Figure~\ref{fig:simulation_results_a2}), the coverage of \texttt{IPTW-SeqGPLVM-impute} falls well below $0.90$, especially in panel (F), corresponding to the $\tau_C = 0.3$ scenario. Thus, in the worst-case regime, the increase in uncertainty is not sufficient to compensate for the additional bias.
...
than the weaker case in terms of the bias while FE-MSM perfroms slightly better across all different $\rho$ values and 
