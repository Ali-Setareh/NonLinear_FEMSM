...
$\mathbb{E}[Y_i(\underline{a}_{T-k})]$ in terms of observed data. 
...
the potential outcome under that treatment history. Formally, if $\bar{A}_{iT}=\bar{a}_{iT}$, 
then $Y_{i}=Y_{i}(\bar{a}_{iT})$. 
Moreover, $Y_{i} = Y_{i}(\underline{a}_{i,T-k})$ if 
$\bar{A}_{iT}=\left(\bar{A}_{i,T-k-1},\underline{a}_{i,T-k}\right)$.
\end{idassumpC}
...
and all histories $(\bar{X}_t, \bar{A}_{t-1}, \eta_i)$ 
with positive probability, every treatment level has positive probability:
\[
\mathbb{P}(A_t = a_t \mid \bar{X}_t = \bar{x}_t,\ \bar{A}_{t-1} = 
\bar{a}_{t-1}, \eta_i) > 0 \quad \text{for all } a_t \in \mathcal{A}_t
\]
Where $\eta_i$ represents the unit-specific fixed effect in the FE-MSM model 
or the confounder substitute in the SeqGPLVM model. This assumption  particularly
is necessary for the identification of $\mathbb{E}[Y_i(\underline{a}_{T-k})]$.
\end{idassumpC}
