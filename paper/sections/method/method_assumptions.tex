...
the potential outcome under that treatment history. Formally, if $\bar{\mathbf{A}}_{iT}=\bar{\mathbf{a}}_{iT}$, 
...
time-invariant. That is, $\mathbf{U}_{it} = \mathbf{U}_i$ for $t \in \{1,\dots,T\}$. We can then write 
...
For all treatment times $t = T-k, \ldots, T$, 
and all histories $(\bar{\mathbf{X}}_{it}, \bar{\mathbf{A}}_{i,t-1}, \boldsymbol{\xi}_i)$ 
with positive probability, every treatment level has positive probability:
\[
\mathbb{P}(A_{it} = a_t \mid \bar{\mathbf{X}}_{it} = \bar{\mathbf{x}}_t,\ \bar{\mathbf{A}}_{i,t-1} = 
\bar{\mathbf{a}}_{t-1}, \boldsymbol{\xi}_i) > 0 \quad \text{for all } a_t \in \mathcal{A}_t
\]
where $\boldsymbol{\xi}_i$ represents the unit-specific fixed effect in FE-MSM 
or the latent variable in the SeqGPLVM model. This assumption is particularly 
necessary for the identification of $\mathbb{E}[Y_i(\underline{\mathbf{a}}_{T-k})]$.
\end{idassumpC}
