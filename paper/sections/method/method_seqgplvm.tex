...
\begin{idassumpSG}[CMM(p): Conditional Markov Model of order p]\label{ass:SG-Marcovianity}
    A latent variable $Z \in \mathbb{R}^d $, a fixed order $p \in \mathbb{N}^*$
    and a parameter vector $\boldsymbol{\gamma} = (\gamma_1, \ldots, \gamma_T)$ exist such that the assigned treatment distribution can be 
    factorized as: 
    \begin{equation}
    p_{\boldsymbol{\gamma}}(\bar{a}_{T} \mid \bar{x}_{T}, z) 
    = \prod_{t=1}^T p_{\gamma_t}(a_{t} \mid a_{t-p:t-1}, \bar{x}_t, z). 
    \label{eq:cmm}
    \end{equation}
...
form of the CMM assumption is stated above. In Eq.~\eqref{eq:cmm},
...
\]    
\end{idassumpSG}
Assumption~\ref{ass:SG-Consistency} implies the uniqueness of the latent variable $Z$ which 
...
The objective of SeqGPLVM is to estimate the joint posterior distribution of the substitute confounders
$\{Z_i\}_{i=1}^N$ given the observed treatment history and covariates,
$p(\{Z_i\}_{i=1}^N \mid \{\bar{A}_{iT}\}_{i=1}^N, \{\bar{X}_{iT}\}_{i=1}^N)$, while imposing the CMM structure on the treatment
assignment mechanism. Here, $Z_i \in \mathbb{R}^d$ denotes the latent confounder substitute
for individual $i$, where $d$ is the dimension of the latent space. To estimate this posterior 
using the 
Bayesian framework, a prior and 
a likelihood model are specified. We assign the following prior to the 
latent confounder substitutes assuming they are i.i.d. across individuals and the latent dimensions:
\[
p(\{z_i\}_{i=1}^N) = \prod_{i=1}^N p(z_i),
...
at each time point: 
\[
f_t : \mathcal{Z} \times \mathcal{X}_{1} \times \cdots \times \mathcal{X}_t \times \mathcal{A}_{t-p} \times \cdots \times \mathcal{A}_{t-1} \to \mathcal{A}_t.
\]
...
we have a lot of flexibility to encode different types of interactions between 
...
\[
p(\{\bar{a}_{iT}\}_{i=1}^N, \bar{f}_T  \mid \{\bar{x}_{iT}\}_{i=1}^N, \{z_i\}_{i=1}^N) = \prod_{i=1}^N 
\prod_{t=1}^T p(a_{it} 
\mid f_t)p(f_t \mid z_i, \bar{x}_{i,t}, \bar{a}_{i,t-p:t-1})).
\]
...
The dimensionality of the inducing points is \(|\mathcal{Z}| + \sum_{s=1}^{t} |\mathcal{X}_s| + \sum_{s=t-p}^{t-1}|\mathcal{A}_{s}|\). 
...
and if \(\mathcal{A}_t\) is continuous, we can use a Gaussian likelihood. The details of the 
variational inference procedure and the optimization of the evidence lower bound (ELBO) are provided 
in \ref{sec:seqgplvm_loss}. In Fig \ref{fig:seqgplvm_architecture} we show 
...
we can use it to adjust for confounding in the outcome model. This stage 
...
$\alpha_i$ replaced by the substitute confounder $Z_i$:
\begin{equation}
...
\pi_{it}
=
\mathbb{P}(A_{it}=1 \mid \bar X_{it}, \bar A_{i,t-1}, Z_i).
\]
...
in the treatment model, we sample from the approximate posterior distribution 
of the substitute confounder $q(z_i)$ and 
...
$z_i^{(s)} \sim q(z_i)$, $s=1,\dots,S$, we re-estimate the propensity scores, construct
the corresponding IPTW weights,
\[
\widehat{W}_i(\hat\beta, z_i^{(s)})
=
\prod_{t=T-K}^{T}
\left(\frac{1}{\pi_{it}(\hat\beta,z_i^{(s)})}\right)^{A_{it}}
\left(\frac{1}{1-\pi_{it}(\hat\beta,z_i^{(s)})}\right)^{1-A_{it}},
\]
...
\left[
Y_i - m\!\left(\underbar{A}_{i,T-k}; \gamma_s \right)
\right].
\]
For each draw $s$, the corresponding MSM parameter estimate $\hat\gamma^{(s)}$ is
obtained by solving
\[
\Psi_N(\gamma_s)=0.
\]
Equivalently, when the MSM is linear and $h(\underbar{A}_{i,T-k})$ is chosen
...
\[
\hat{\gamma} = \frac{1}{S}\sum_{s=1}^{S}\hat{\gamma}^{(s)}.
\]

This procedure introduces two sources of uncertainty: first, uncertainty about the
latent substitute confounder induced by the approximate posterior $q(z_i)$, and second,
