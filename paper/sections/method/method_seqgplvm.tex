...
$V^{(s)}$ denote the robust (sandwich) covariance estimate associated with
$\hat{\gamma}^{(s)}$. Then the pooled variance estimator is
\[
W = \frac{1}{S}\sum_{s=1}^{S} V^{(s)},
\qquad
B = \mathrm{Var}_s\!\left(\hat{\gamma}^{(s)}\right),
\qquad
T = W + \left(1+\frac{1}{S}\right)B.
\]
The resulting confidence interval $T$ therefore reflect both
