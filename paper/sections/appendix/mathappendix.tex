...
\subsection{Notation}
We use uppercase letters for random quantities and 
the corresponding lowercase letters for their realizations. 
Boldface denotes vectors or collections, and plain letters 
denote scalars. Individuals are indexed by \(i \in \{1,\dots,N\}\) 
and time by \(t \in \{1,\dots,T\}\). Let \(\mathbf{X}_{it} \in \mathbb{R}^D\) denote the 
covariate vector for individual \(i\) at time \(t\), 
and let \(A_{it}\) denote the treatment at time \(t\). 
For simplicity, we assume scalar treatments, but the extension to multi-dimensional 
treatments is straightforward. 
...
$
The full covariate and treatment collections are denoted by
$
...
Let \(\mathbf{Z} := \{\mathbf{Z}_i\}_{i=1}^N\) denote 
the collection of time-invariant latent variables, 
where \(\mathbf{Z}_i \in \mathbb{R}^J\) and \(J\) is the 
latent dimension. Finally, \(\boldsymbol{\gamma}\) denotes generative 
