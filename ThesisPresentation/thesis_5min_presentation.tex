...
\begin{itemize}
  ...
  \item Create a weighted pseudo-population where treatment looks randomized.
...
E\left[
\frac{
\mathbf{1}(\bar A_{iT}=\bar a_T)Y_i
}{
\prod_{t=1}^{T}
\pi_{it}^{a_t}(1-\pi_{it})^{1-a_t}
}
\right]
\]
}
...
\pstrong{Step 3: Fit the weighted marginal structural model (MSM)}
\[
...
\begin{frame}{Estimation equations: FE-MSM}
\vspace{-0.25cm}
{\fontsize{7.6}{9.0}\selectfont

\textbf{Target}
\[
E\{Y_i(a_{T-k})\}=E\{Y_i(\overline A_{i,T-k-1},a_{T-k})\}.
\]

\textbf{Fixed-effect propensity model}
\[
\pi_{it}=P(A_{it}=1\mid \overline X_{it},\overline A_{i,t-1},\alpha_i).
\]

\[
\ell_{it}(\beta,\alpha_i)
=
A_{it}\log \pi_{it}(\beta,\alpha_i)
+
(1-A_{it})\log\{1-\pi_{it}(\beta,\alpha_i)\}.
\]
...
\textbf{IPTW weights for the truncated regime}
\[
\widehat W_i(\hat\beta,\hat\alpha_i)
=
\prod_{t=T-k}^{T}
...
\textbf{Weighted MSM estimating equation}
\[
\Psi_N(\theta)
=
\frac{1}{N}\sum_{i=1}^N
...
{\fontsize{11}{14}\selectfont\teal{What the thesis clarifies}}\par
\vspace{0.25cm}

{\fontsize{9.4}{12.0}\selectfont
\spacedparagraph{FE-MSM and SeqGPLVM-MSM are not two completely separate pipelines: both can be understood as extensions of IPTW-MSM.}

\spacedparagraph{For a fair comparison, both methods are evaluated for the same truncated treatment-regime effect ($E[Y_i(a_{T-k})]$).}

\spacedparagraph{The key difference is how they represent the stable hidden factor: FE-MSM uses a unit fixed effect ($\alpha_i$), while SeqGPLVM learns a latent substitute ($Z_i$).}
}
\end{column}

\begin{column}{0.48\textwidth}
{\fontsize{11}{14}\selectfont\teal{What the comparison shows}}\par
\vspace{0.25cm}

{\fontsize{9.4}{12.0}\selectfont
\spacedparagraph{Ignoring the time-invariant confounder ($U_i$) leaves standard IPTW biased in the simulation setting.}

\spacedparagraph{Both adjusted methods reduce bias, especially as the number of individuals increases.}

\spacedparagraph{FE-MSM is more stable in this DGP; SeqGPLVM-MSM is more flexible, but depends more strongly on latent identifiability and enough treatment history.}
}
\end{column}
...
{\fontsize{10.2}{13.5}\selectfont\RaggedRight
\red{Bottom line:} these methods can help under stable hidden confounding, but they do not remove the need for credible structural assumptions and careful diagnostics.\par}
...
\begin{frame}{Appendix: Simulation-specific SeqGPLVM-MSM}
\vspace{-0.2cm}
{\fontsize{7.2}{8.6}\selectfont

\textbf{SeqGPLVM treatment model}

\[
Z_i\in\mathbb{R},
\qquad
Z_i\sim \mathrm{CMM}(1).
\]

\[
p_\gamma(\overline a_{iT}\mid \overline x_{iT},z_i)
=
\prod_{t=1}^{T}
p_{\gamma_t}
\left(
a_{it}\mid x_{it},a_{i,t-1},z_i
\right).
\]

\textbf{Linear kernel used in the simulation}

\[
k^{\text{treat}}_t
\left(
(x_t,a_{t-1},z),(x'_t,a'_{t-1},z')
\right)
=
\nu
\left[
\sum_{j=1}^{|X_t|}x_{jt}x'_{jt}
+
a_{t-1}a'_{t-1}
+
zz'
...
\textbf{Propensity model after drawing latent variables}

\[
z_i^{(s)}\sim q_\psi(Z_i),
\qquad s=1,\ldots,S,
\qquad S=100.
\]
...
\begin{frame}{Appendix: Simulation-specific estimation models}
\vspace{-0.2cm}
{\fontsize{7.4}{8.8}\selectfont

\textbf{Common weighted MSM}
...
\sum_{t=T-3}^{T-1} a_t ,
\qquad
\theta=(\alpha',\tau'_F,\tau'_C).
\]

\[
(\hat\tau_F,\hat\tau_C)
=
\arg\min_{\tau'_F,\tau'_C}
\sum_{i=1}^N
\widehat W_i
\left[
Y_i-\alpha'
-\tau'_F A_{iT}
-\tau'_C\sum_{t=T-3}^{T-1}A_{it}
\right]^2 .
\]

\vspace{0.15cm}

\textbf{FE-MSM propensity model}

\[
P(A_{it}=1\mid X_{it},A_{i,t-1},\alpha_i)
=
\operatorname{expit}
\left(
w_1^\top X_{it}
...
\textbf{Stabilized-weight numerator model}

\[
P(A_{it}=1\mid A_{i,t-1})
=
\operatorname{expit}
\left(
w_7 A_{i,t-1}+w_8
...
\begin{frame}{Appendix: how SeqGPLVM estimates the latent variable}
\vspace{-0.25cm}
{\fontsize{6.9}{8.2}\selectfont

\textbf{Goal: infer a time-invariant substitute confounder}
\[
Z_i\in\mathbb{R},
\qquad
q_\psi(Z)
=
\prod_{i=1}^{N}\mathcal N(z_i\mid \mu_i,\sigma_i^2).
\]

\textbf{Treatment sequence model}
\[
p_\gamma(\bar a_{iT}\mid \bar x_{iT},z_i)
=
\prod_{t=1}^{T}
p_{\gamma_t}(a_{it}\mid x_{it},a_{i,t-1},z_i).
\]

\textbf{GP treatment likelihood}
\[
f_t(x_{it},a_{i,t-1},z_i)\sim GP(0,k_t),
\qquad
P(A_{it}=1\mid f_{it})
=
\sigma(f_{it}).
...
\[
\log p_{\gamma_t}(A_{it}\mid f_{it})
=
A_{it}\log \sigma(f_{it})
+
(1-A_{it})\log\{1-\sigma(f_{it})\}.
\]
...
\left[
\sum_{i=1}^{N}
\log p_{\gamma_t}(A_{it}\mid x_{it},a_{i,t-1},z_i)
\right]
-
\mathrm{KL}
\left[
q_\psi(Z)\,\Vert\,p_\gamma(Z)
\right].
\]
